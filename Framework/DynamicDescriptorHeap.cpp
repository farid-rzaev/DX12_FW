...
DynamicDescriptorHeap::DynamicDescriptorHeap(std::shared_ptr<Application> app, D3D12_DESCRIPTOR_HEAP_TYPE heapType, uint32_t numDescriptorsPerHeap)
    ...
    , m_NumFreeHandles(0)
{
    m_DescriptorHandleIncrementSize = m_Application->GetDescriptorHandleIncrementSize(heapType);
...
void DynamicDescriptorHeap::ParseRootSignature(const RootSignature& rootSignature)
{
    ...
    // command list.
    // -- 
    // Reseting mask to indicate that no descriptors should be copied to a GPU visible
    // descriptor heap until new descriptors are staged to the DynamicDescriptorHeap.
    m_StaleDescriptorTableBitMask = 0;
    ...
    // --
    // Bits are set only for descriptor table root parameters, but not for root constants.
