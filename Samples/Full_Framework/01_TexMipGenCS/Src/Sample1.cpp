...
// --
// Max size of each CB slot is 64 DWords = 64 * 8 Bytes = 512 Bytes
// * As size(MatricesCB) = 4 * (16 * 4 Bytes) = 256 Bytes is only a half of a CB's max size, 
//   we can put it to reggister space0 (if reg space is not specified it's space0 by default).
//   And use space1 for another CB, for example, for Material CB as it occupies only 80 Bytes 
//   out of the 256 remaining Bytes in the space1. 
enum RootParameters
{
...
void Sample1::OnMouseMoved( MouseMotionEventArgs& e )
{
    Game::OnMouseMoved( e );
